...
export function Caricamento({ testo = "Caricamento…" }: { testo?: string }) {
  return (
    <div
      className="flex items-center gap-3 py-10 text-sm text-muted"
      role="status"
    >
      <span
        aria-hidden="true"
...
export function Vuoto({ titolo, descrizione, azione }: VuotoProps) {
  return (
    <div>
      ...
      {descrizione && (
        <p className="mt-1.5 max-w-[52ch] text-sm text-muted">{descrizione}</p>
      )}
...
export function Errore({ errore, onRiprova }: ErroreProps) {
  return (
    <div
      className="border-l-2 border-scaduta bg-scaduta-soft py-4 pl-4 pr-4"
      role="alert"
    >
      <p className="text-sm font-medium text-scaduta">
