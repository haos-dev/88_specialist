...
const schema = z.object({
  ...
  email: opzionale.refine((v) => v === null || z.string().email().safeParse(v).success, {
...
export function ClientForm({ aperto, cliente, inCorso, onSalva, onChiudi }: ClientFormProps) {
      ...
      azioni={
        <>
          ...
          <Button variante="primario" onClick={() => void invia()} disabled={inCorso}>
            {inCorso ? 'Salvataggio…' : modifica ? 'Salva modifiche' : 'Crea cliente'}
          </Button>
        </>
      }
    >
      <form onSubmit={invia} className="flex flex-col gap-4" noValidate>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Cognome" obbligatorio errore={errors.last_name?.message}>
            {(props) => (
              <Input
                {...props}
                {...register('last_name')}
