...
const schema = z
  .object({
    client_id: z
      .string()
      .trim()
      .min(1, "Scegli il cliente a cui destinare la scheda."),
  ...
  return (
    ...
    >
      <form onSubmit={invia} className="flex flex-col gap-4" noValidate>
        ...
        <Field
          label="Titolo della nuova scheda"
          obbligatorio
          errore={errors.titolo?.message}
        >
          {(props) => (
            <Input
              {...props}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Inizio" errore={errors.inizio?.message}>
            {(props) => (
              <Input {...props} {...register("inizio")} type="date" />
            )}
          </Field>
          <Field label="Fine" errore={errors.fine?.message}>
            {(props) => (
              <Input
                {...props}
                {...register("fine")}
                type="date"
                aria-invalid={Boolean(errors.fine)}
              />
            )}
          </Field>
        </div>
